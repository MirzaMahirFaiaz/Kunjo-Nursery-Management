...
create or replace package insert_product_package AS
    procedure insert_product_into_site1(branch_id in Branch.BranchID%type,p_name in Product.product_name%type,p_desc in Product.product_description%type,p_model in Product.product_origin%type,p_brand in Product.product_type%type,p_purchase in Product.product_purchase_rate%type,p_sales in Product.product_sales_rate%type,p_quantity in Product.total_quantity%type);
...
create or replace package body insert_product_package as
    ...
    IS
    BEGIN
        insert into Product1(product_name,product_description,product_origin,product_type,
    ...
    IS
    BEGIN
        insert into Product2@site2(product_name,product_description,product_origin,product_type,
        product_purchase_rate,product_sales_rate,total_quantity,BranchID) values 
        (p_name,p_desc,p_model,p_brand,p_purchase,p_sales,p_quantity,branch_id);
		commit;
...
ACCEPT ProductOriginInput PROMPT "Enter product model:";
ACCEPT ProductTypeInput PROMPT "Enter product brand:";
...
ProductOrigin Product.product_origin%type := '&ProductOriginInput';
ProductType Product.product_type%type := '&ProductTypeInput';
...
begin 
    ...
    if BranchId = 1 THEN
        insert_product_package.insert_product_into_site1(BranchId,ProductName,ProductDesc,ProductOrigin,ProductType,ProductPurchaseRate,ProductSalesRate,ProductTotalQuantity);
    elsif BranchId = 2 THEN
        insert_product_package.insert_product_into_site2(BranchId,ProductName,ProductDesc,ProductOrigin,ProductType,ProductPurchaseRate,ProductSalesRate,ProductTotalQuantity);
    end if;
